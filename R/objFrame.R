...
setMethod("names", "EnrichGT_obj", function(x) {
  if (!is.data.frame(x@enriched_result)) {
    return
  }
  NULL
...
#' @importFrom htmltools HTML div h1 h2 h3 h4 p
...
summary_use_local_html <- function(x, name) {
  ...
  if (length(resTable$Description) == 0) {
    return(htmltools::div(
      style = "color: #dc3545; margin: 15px;",
      "Please check your input. For example, you can use Object$C1, Object$c1 or Object$Cluster_1 to reach the cluster 1's result summary"
    ))
  }
  CandidateGenes <- paste(x@gene_modules[[name]], collapse = ", ")
  ...
  htmltools::div(
    style = "font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; margin: 20px;",
    htmltools::h2(
      style = "color: #495057; border-bottom: 2px solid #007bff; padding-bottom: 10px; margin-bottom: 20px;",
      htmltools::HTML(glue::glue("Enrichment Result of {name} (Local Summary)"))
    ),
    htmltools::div(
      style = "background-color: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 15px;",
      htmltools::p(
        style = "color: #495057; line-height: 1.6; margin-bottom: 10px;",
        htmltools::HTML(glue::glue(
          "<strong>This cluster contains:</strong> {DescriptPrint} ..."
        ))
      ),
      htmltools::p(
        style = "color: #495057; line-height: 1.6;",
        htmltools::HTML(glue::glue(
          "<strong>Candidate genes include:</strong> {CandidateGenes}"
        ))
      )
    ),
    htmltools::div(
      style = "color: #6c757d; font-size: 0.9em; font-style: italic;",
  ...
  tryCatch(
    {
      ...
      htmltools::div(
        style = "font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; margin: 20px;",
        htmltools::h2(
          style = "color: #495057; border-bottom: 2px solid #28a745; padding-bottom: 10px; margin-bottom: 20px;",
          htmltools::HTML(glue::glue(
            "Enrichment Result of {name} (LLM Summary)"
          ))
        ),
        htmltools::div(
          style = "background-color: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 20px;",
          htmltools::h3(style = "color: #28a745; margin-bottom: 15px;", title),
          htmltools::div(
            ...
            htmltools::div(
              style = "color: #495057; line-height: 1.6; white-space: pre-wrap; background-color: white; padding: 15px; border-radius: 5px; border-left: 4px solid #17a2b8;",
              htmltools::HTML(gsub("\n", "<br>", pathway_summary))
            )
          ),
          htmltools::div(
            ...
            htmltools::h4(
              style = "color: #6f42c1; margin-bottom: 10px;",
              "Gene Analysis:"
            ),
            htmltools::div(
              style = "color: #495057; line-height: 1.6; white-space: pre-wrap; background-color: white; padding: 15px; border-radius: 5px; border-left: 4px solid #6f42c1;",
              htmltools::HTML(gsub("\n", "<br>", gene_summary))
            )
          )
        )
  ...
  tryCatch(
    {
      ...
      if (is.null(comparison_data)) {
        cli::cli_alert_warning(paste0(
          "No comparison data found for cluster ",
          name
        ))
        return(summary_use_local(x, name))
      }
      ...
      for (model in model_names) {
        if (!is.null(comparison_data$model_results[[model]])) {
          ...
          ul1 <- cli::cli_ul()
          cli::cli_li(glue::glue(
            "Title: {comparison_data$model_results[[model]]$title}"
          ))
          cli::cli_li(glue::glue(
            "Pathway Analysis: {comparison_data$model_results[[model]]$pathway_summary}"
          ))
          cli::cli_li(glue::glue(
            "Gene Analysis: {comparison_data$model_results[[model]]$gene_summary}"
          ))
          cli::cli_end(ul1)
        }
      }
    ...
    error = function(e) {
      cli::cli_alert_danger(
        "Error displaying LLM comparison results. Falling back to local summary."
      )
      summary_use_local(x, name)
    }
  )
  ...
  tryCatch(
    {
      ...
      model_cards <- lapply(model_names, function(model) {
        if (!is.null(comparison_data$model_results[[model]])) {
          htmltools::div(
            style = "background-color: white; padding: 20px; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); border: 1px solid #e9ecef; height: 100%; display: flex; flex-direction: column;",
            htmltools::h4(
              style = "color: #e83e8c; margin-bottom: 15px; padding-bottom: 8px; border-bottom: 2px solid #e83e8c; font-size: 16px;",
            ...
            htmltools::div(
              ...
              htmltools::div(
                ...
                htmltools::p(
                  style = "color: #495057; line-height: 1.5; margin-bottom: 8px; font-size: 14px;",
                  htmltools::HTML(glue::glue(
                    "<strong>Title:</strong> {comparison_data$model_results[[model]]$title}"
                  ))
                )
              ),
              htmltools::div(
                style = "margin-bottom: 12px;",
                htmltools::p(
                  style = "color: #495057; line-height: 1.5; margin-bottom: 5px; font-size: 14px; font-weight: 600;",
                  "Pathway Analysis:"
                ),
                htmltools::p(
                  style = "color: #495057; line-height: 1.5; font-size: 13px; background-color: #f8f9fa; padding: 8px; border-radius: 4px; margin: 0;",
                  htmltools::HTML(glue::glue(
                    "{comparison_data$model_results[[model]]$pathway_summary}"
                  ))
                )
              ),
              htmltools::div(
                ...
                htmltools::p(
                  style = "color: #495057; line-height: 1.5; font-size: 13px; background-color: #f8f9fa; padding: 8px; border-radius: 4px; margin: 0;",
                  htmltools::HTML(glue::glue(
                    "{comparison_data$model_results[[model]]$gene_summary}"
                  ))
                )
              )
            )
      ...
      htmltools::div(
        style = "font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; margin: 20px;",
        htmltools::h2(
          style = "color: #495057; border-bottom: 2px solid #fd7e14; padding-bottom: 10px; margin-bottom: 20px;",
  ...
  tryCatch(
    {
      ...
      cli::cli_h1(glue::glue(
        "Enrichment Result of {name} (LLM Summary - {model_name})"
      ))
      ...
      if (length(cluster_idx) == 0) {
        cli::cli_alert_warning(paste0(
          "No LLM summary found for cluster ",
          name
        ))
        return(summary_use_local(x, name))
      }
    ...
    error = function(e) {
      cli::cli_alert_danger(
        "Error displaying single LLM comparison results. Falling back to local summary."
      )
      summary_use_local(x, name)
    }
  )
  ...
  tryCatch(
    {
      ...
      htmltools::div(
        style = "font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; margin: 20px;",
        htmltools::h2(
          style = "color: #495057; border-bottom: 2px solid #6f42c1; padding-bottom: 10px; margin-bottom: 20px;",
          htmltools::HTML(glue::glue(
            "Enrichment Result of {name} (LLM Summary - {model_name})"
          ))
        ),
        ...
        htmltools::div(
          style = "background-color: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 20px;",
          htmltools::h3(style = "color: #6f42c1; margin-bottom: 15px;", title),

          htmltools::div(
            style = "background-color: white; padding: 15px; border-radius: 5px; margin-bottom: 15px; border-left: 4px solid #28a745;",
            htmltools::h4(
          ...
          htmltools::div(
            style = "background-color: white; padding: 15px; border-radius: 5px; border-left: 4px solid #6c757d;",
            htmltools::h4(
              style = "color: #6c757d; margin-bottom: 10px;",
              "Model Information"
            ),
            htmltools::p(
              style = "color: #495057; line-height: 1.5; margin-bottom: 5px;",
              htmltools::HTML(glue::glue(
                "<strong>Model:</strong> {model_name}"
              ))
            ),
            htmltools::p(
              style = "color: #495057; line-height: 1.5;",
              htmltools::HTML(glue::glue(
                "<strong>Analysis Time:</strong> {llm_result@llm_model_info}"
              ))
            )
          )
        )
