...
df00<-data.frame(Package=c("simplifyEnrichment","EnrichGT","aPEAR"),Time=c(10.1,t2,t3))# In simplifyEnrichment, it will cluster word cloud. We don't Calculate this part of time

df00
```

```{r}
ggplot(df00,aes(x=Package,y=Time))+geom_col(aes(fill=Package))+theme_bw()
